...
//   1: if nodeID --> nbrID
//   2: if nodeID <-- nbrID
int checkEdge(PNGraph& G, int64 nodeID, int64 nbrID) {
...
int checkTriadMotif(PNGraph& G, int64 nodeID, int64 srcNId, int64 dstNId) {
  int motifType = 0;
...
void ProcessedGraph::countDirTriadMotif(PNGraph graph) {
  ...
  for (TNGraph::TNodeI NI = graph->BegNI(); NI < graph->EndNI(); NI ++ ) {
    ...
    for (TNGraph::TEdgeI EI = subGraph->BegEI(); EI < subGraph->EndEI(); EI ++ ) { 
      int64 srcNId = EI.GetSrcNId();
      int64 dstNId = EI.GetDstNId();
      int MotifNumber = 0;
...
void MAPPR::computeProfile(const ProcessedGraph& graph_p) {
  ...
  for (THash<TInt, TFlt>::TIter it = Quotient.BegI(); it < Quotient.EndI(); it++) {
    ...
    for (int64 j = 0; j < NI.GetOutDeg(); j ++) {
      int64 NbrId = NI.GetOutNId(j);
      if (IsIn.IsKey(NbrId)) {
