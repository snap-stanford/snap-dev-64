...
template <class PGraph> int64 CntInDegNodes(const PGraph& Graph, const int64& NodeInDeg);
/// Returns the number of nodes with out-degree NodeOutDeg.
template <class PGraph> int64 CntOutDegNodes(const PGraph& Graph, const int64& NodeOutDeg);
/// Returns the number of nodes with degree NodeDeg.
template <class PGraph> int64 CntDegNodes(const PGraph& Graph, const int64& NodeDeg);
/// Returns the number of nodes with degree greater than 0.
template <class PGraph> int64 CntNonZNodes(const PGraph& Graph);
/// Returns the number of nodes in NodeSet that have an edge to the node NId.
template <class PGraph> int64 CntEdgesToSet(const PGraph& Graph, const int64& NId, const TInt64Set& NodeSet);

/// Returns a randomly chosen node from all the nodes with the maximum degree.
template <class PGraph> int64 GetMxDegNId(const PGraph& Graph);
/// Returns a randomly chosen node from all the nodes with the maximum in-degree.
template <class PGraph> int64 GetMxInDegNId(const PGraph& Graph);
/// Returns a randomly chosen node from all the nodes with the maximum out-degree.
template <class PGraph> int64 GetMxOutDegNId(const PGraph& Graph);

// degree histograms
/// Returns an in-degree histogram: a set of pairs <code>(in-degree, number of nodes of such in-degree)</code>
template <class PGraph> void GetInDegCnt(const PGraph& Graph, TIntPr64V& DegToCntV);
/// Returns an in-degree histogram: a set of pairs <code>(in-degree, number of nodes of such in-degree)</code>
template <class PGraph> void GetInDegCnt(const PGraph& Graph, TFltPr64V& DegToCntV);
/// Returns an out-degree histogram: a set of pairs <code>(out-degree, number of nodes of such out-degree)</code>
template <class PGraph> void GetOutDegCnt(const PGraph& Graph, TIntPr64V& DegToCntV);
/// Returns an out-degree histogram: a set of pairs <code>(out-degree, number of nodes of such out-degree)</code>
template <class PGraph> void GetOutDegCnt(const PGraph& Graph, TFltPr64V& DegToCntV);
/// Returns a degree histogram: a set of pairs <code>(degree, number of nodes of such degree)</code>
template <class PGraph> void GetDegCnt(const PGraph& Graph, TIntPr64V& DegToCntV);
/// Returns a degree histogram: a set of pairs <code>(degree, number of nodes of such degree)</code>
template <class PGraph> void GetDegCnt(const PGraph& Graph, TFltPr64V& DegToCntV);
/// Returns a degree sequence vector.
template <class PGraph> void GetDegSeqV(const PGraph& Graph, TInt64V& DegV);
/// Returns an in- and out-degree sequence vectors.
template <class PGraph> void GetDegSeqV(const PGraph& Graph, TInt64V& InDegV, TInt64V& OutDegV);

/// Returns a vector of pairs <code>(node id, node in-degree)</code>
template <class PGraph> void GetNodeInDegV(const PGraph& Graph, TIntPr64V& NIdInDegV);
/// Returns a vector of pairs <code>(node id, node out-degree)</code>
template <class PGraph> void GetNodeOutDegV(const PGraph& Graph, TIntPr64V& NIdOutDegV);
...
template <class PGraph> int64 CntUniqUndirEdges(const PGraph& Graph);
...
template <class PGraph> int64 CntUniqDirEdges(const PGraph& Graph);
...
template <class PGraph> int64 CntUniqBiDirEdges(const PGraph& Graph);
/// Counts the number of self-edges in a graph. Edge <code>(u,u)</code> is a self-edge.
template <class PGraph> int64 CntSelfEdges(const PGraph& Graph);
...
template <class PGraph> void DelNodes(PGraph& Graph, const TInt64V& NIdV);
...
template <class PGraph> void DelDegKNodes(PGraph& Graph, const int64& OutDegK, const int64& InDegK);

/////////////////////////////////////////////////
// Tree Routines
template <class PGraph> bool IsTree(const PGraph& Graph, int64& RootNIdX);
template <class PGraph> int64 GetTreeRootNId(const PGraph& Graph) { int64 RootNId; bool Tree; Tree = IsTree(Graph, RootNId);  Assert(Tree);  return RootNId; }
template <class PGraph> void GetTreeSig(const PGraph& Graph, const int64& RootNId, TInt64V& Sig);
template <class PGraph> void GetTreeSig(const PGraph& Graph, const int64& RootNId, TInt64V& Sig, TIntPr64V& NodeMap);

/////////////////////////////////////////////////
// Implementation
template <class PGraph>
int64 CntInDegNodes(const PGraph& Graph, const int64& NodeInDeg) {
  int64 Cnt = 0;
...
template <class PGraph>
int64 CntOutDegNodes(const PGraph& Graph, const int64& NodeOutDeg) {
  int64 Cnt = 0;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
int64 CntDegNodes(const PGraph& Graph, const int64& NodeDeg) {
  int64 Cnt = 0;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
int64 CntNonZNodes(const PGraph& Graph) {
  int64 Cnt = 0;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    if (NI.GetDeg() > 0) Cnt++;
  }
...
template <class PGraph>
int64 CntEdgesToSet(const PGraph& Graph, const int64& NId, const TInt64Set& NodeSet) {
...
template <class PGraph>
int64 GetMxDegNId(const PGraph& Graph) {
  TInt64V MxDegV;
  int64 MxDeg=-1;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
int64 GetMxInDegNId(const PGraph& Graph) {
  TInt64V MxDegV;
  int64 MxDeg=-1;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
int64 GetMxOutDegNId(const PGraph& Graph) {
  TInt64V MxDegV;
  int64 MxDeg=-1;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
void GetInDegCnt(const PGraph& Graph, TIntPr64V& DegToCntV) {
  TInt64H DegToCntH;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    DegToCntH.AddDat(NI.GetInDeg())++; }
  DegToCntV.Gen(DegToCntH.Len(), 0);
  for (int64 i = 0; i < DegToCntH.Len(); i++) {
    DegToCntV.Add(TInt64Pr(DegToCntH.GetKey(i), DegToCntH[i])); }
  DegToCntV.Sort();
}

template <class PGraph>
void GetInDegCnt(const PGraph& Graph, TFltPr64V& DegToCntV) {
  TInt64H DegToCntH;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    DegToCntH.AddDat(NI.GetInDeg())++; }
  DegToCntV.Gen(DegToCntH.Len(), 0);
...
template <class PGraph>
void GetOutDegCnt(const PGraph& Graph, TIntPr64V& DegToCntV) {
  TInt64H DegToCntH;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
void GetOutDegCnt(const PGraph& Graph, TFltPr64V& DegToCntV) {
  TInt64H DegToCntH;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    DegToCntH.AddDat(NI.GetOutDeg())++; }
  DegToCntV.Gen(DegToCntH.Len(), 0);
...
template <class PGraph>
void GetDegCnt(const PGraph& Graph, TIntPr64V& DegToCntV) {
  TInt64H DegToCntH;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
void GetDegCnt(const PGraph& Graph, TFltPr64V& DegToCntV) {
  TInt64H DegToCntH;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    DegToCntH.AddDat(NI.GetDeg())++; }
  DegToCntV.Gen(DegToCntH.Len(), 0);
...
template <class PGraph>
void GetDegSeqV(const PGraph& Graph, TInt64V& DegV) {
...
template <class PGraph>
void GetDegSeqV(const PGraph& Graph, TInt64V& InDegV, TInt64V& OutDegV) {
...
template <class PGraph>
void GetNodeInDegV(const PGraph& Graph, TIntPr64V& NIdInDegV) {
  NIdInDegV.Reserve(Graph->GetNodes(), 0);
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    NIdInDegV.Add(TInt64Pr(NI.GetId(), NI.GetInDeg()));
  }
}

template <class PGraph>
void GetNodeOutDegV(const PGraph& Graph, TIntPr64V& NIdOutDegV) {
  NIdOutDegV.Reserve(Graph->GetNodes(), 0);
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    NIdOutDegV.Add(TInt64Pr(NI.GetId(), NI.GetOutDeg()));
  }
}

template <class PGraph>
int64 CntUniqUndirEdges(const PGraph& Graph) {
  TInt64Set NbrSet;
  TInt64Set SelfNbrSet;
  int64 Cnt = 0;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    NbrSet.Clr(false);
    for (int64 e = 0; e < NI.GetDeg(); e++) { // unique neighbors of a node
      const int64 NbrId = NI.GetNbrNId(e);
...
template <class PGraph>
int64 CntUniqDirEdges(const PGraph& Graph) {
  TInt64Set NbrSet;
  int64 Cnt = 0;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    NbrSet.Clr(false);
    for (int64 e = 0; e < NI.GetOutDeg(); e++) { // unique out-neighbors of a node
...
template <class PGraph>
int64 CntUniqBiDirEdges(const PGraph& Graph) {
  ...
  int64 Cnt = 0;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    const int64 SrcId = NI.GetId();
    for (int64 e = 0; e < NI.GetOutDeg(); e++) {
      const int64 DstId = NI.GetOutNId(e);
...
template <class PGraph>
int64 CntSelfEdges(const PGraph& Graph) {
  int64 Cnt = 0;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
    for (int64 e = 0; e < NI.GetOutDeg(); e++) {
...
void MakeUnDir(const PGraph& Graph) {
  ...
  for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++) {
    ...
    if (! Graph->IsEdge(DstNId, SrcNId)) {
      EdgeV.Add(TInt64Pr(DstNId, SrcNId));
    }
  }
...
struct TDelSelfEdges<PGraph, true> { // mutligraph specialization
  static void Do(const PGraph& Graph) {
    ...
    for (int64 i = 0; i < EdgeV.Len(); i++) { // delete all edges between a pair of nodes
...
template <class PGraph>
void DelNodes(PGraph& Graph, const TInt64V& NIdV) {
...
void DelZeroDegNodes(PGraph& Graph) {
  ...
  for (int64 i = 0; i < DelNIdV.Len(); i++) {
    Graph->DelNode(DelNIdV[i]);
  }
}

template <class PGraph>
void DelDegKNodes(PGraph& Graph, const int64& OutDegK, const int64& InDegK) {
  TInt64V DelNIdV;
  for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
...
template <class PGraph>
bool IsTree(const PGraph& Graph, int64& RootNId) {
...
template <class PGraph>
void GetTreeSig(const PGraph& Graph, const int64& RootNId, TInt64V& Sig) {
  CAssert(HasGraphFlag(typename PGraph::TObj, gfDirected));
  Sig.Gen(Graph->GetNodes(), 0);
  TSnapQueue<int64, int64> NIdQ(Graph->GetNodes());
  NIdQ.Push(RootNId);
  int64 LastPos = 0, NodeCnt = 1;
  while (! NIdQ.Empty()) {
    ...
    if (--NodeCnt == 0) {
      for (int64 i = LastPos; i < Sig.Len(); i++) NodeCnt += Sig[i];
...
// tree signature -- for each level we sort the node in-degrees and concatenate them into a vector
template <class PGraph>
void GetTreeSig(const PGraph& Graph, const int64& RootNId, TInt64V& Sig, TIntPr64V& NodeMap) {
  CAssert(HasGraphFlag(typename PGraph::TObj, gfDirected));
  ...
  while (! NIdQ.Empty()) {
    ...
    if (Node.GetInDeg() != 0) {
      for (int64 e = 0; e < Node.GetInDeg(); e++) {
        NIdQ.Push(Node.GetInNId(e)); }
      NodeMap.Add(TInt64Pr(Node.GetInDeg(), Node.GetId()));
    } else if (Node.GetOutDeg() != 0) {
      for (int64 e = 0; e < Node.GetOutDeg(); e++) {
        NIdQ.Push(Node.GetOutNId(e)); }
      NodeMap.Add(TInt64Pr(Node.GetOutDeg(), Node.GetId()));
    }
    if (--NodeCnt == 0) {
      for (int64 i = LastPos; i < NodeMap.Len(); i++) {
        NodeCnt += NodeMap[i].Val1; }
      NodeMap.QSort(LastPos, NodeMap.Len()-1, false);
      LastPos = NodeMap.Len();
    }
  }
  for (int64 i = 0; i < NodeMap.Len(); i++) {
