...
private:
  ...
  TInt64V InfectNIdV;  // nodes to start fire
  TInt64V BurnedNIdV;  // nodes that got burned (FF model creates edges to them)
  // statistics
  TInt64V NBurnedTmV, NBurningTmV, NewBurnedTmV; // total burned, currently burning, newly burned in current time step
private:
  UndefCopyAssign(TForestFire);
public:
  TForestFire() : Rnd(1), Graph(), FwdBurnProb(0.0), BckBurnProb(0.0), ProbDecay(1.0) { }
  TForestFire(const PNGraph& GraphPt, const double& ForwBurnProb, const double& BackBurnProb, const double& DecayProb=1.0, const int64& RndSeed=1) :
  ...
  void Infect(const int64& NodeId) { InfectNIdV.Gen(1,1);  InfectNIdV[0] = NodeId; }
  void Infect(const TInt64V& InfectedNIdV) { InfectNIdV = InfectedNIdV; }
  void InfectAll();
  void InfectRnd(const int64& NInfect);
  ...
  int64 GetFireTm() const { return NBurnedTmV.Len(); } // time of fire
  int64 GetBurned() const { return BurnedNIdV.Len(); }
  int64 GetBurnedNId(const int64& NIdN) const { return BurnedNIdV[NIdN]; }
  const TInt64V& GetBurnedNIdV() const { return BurnedNIdV; }
  void GetBurnedNIdV(TInt64V& NIdV) const { NIdV = BurnedNIdV; }
  void PlotFire(const TStr& FNmPref, const TStr& Desc, const bool& PlotAllBurned=false);

  static PNGraph GenGraph(const int64& Nodes, const double& FwdProb, const double& BckProb);
};
...
public:
  ...
  static int64 TimeLimitSec;
private:
  PNGraph Graph;
  // parameters
  TBool BurnExpFire;   // burn Exponential or Geometric fire
  TInt64 StartNodes;     // start a graph with N isolated nodes
  TFlt FwdBurnProb, BckBurnProb, ProbDecay; // Forest Fire parameters
  TFlt Take2AmbProb, OrphanProb;
public:
  TFfGGen(const bool& BurnExpFireP, const int64& StartNNodes, const double& ForwBurnProb,
  ...
  TStopReason AddNodes(const int64& GraphNodes, const bool& FloodStop = true);
  TStopReason GenGraph(const int64& GraphNodes, const bool& FloodStop = true);
  TStopReason GenGraph(const int64& GraphNodes, PGStatVec& EvolStat, const bool& FloodStop=true);
...
private:
  ...
  PUNGraph Graph;
  double BurnProb;
  TInt64Set BurnedSet; // all burned nodes in the current iteration
  TInt64V BurningNIdV, NewBurnedNIdV, AliveNIdV; // temporary
public:
  ...
  int64 GetNBurned() const { return BurnedSet.Len(); }
  int64 GetBurnedNId(const int64& n) const { return BurnedSet[n]; }
  int64 BurnGeoFire(const int64& StartNId);
  TFfGGen::TStopReason AddNodes(const int64& GraphNodes, const bool& FloodStop=true);
};
