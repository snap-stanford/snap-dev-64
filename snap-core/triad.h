...
template <class PGraph> double GetClustCf(const PGraph& Graph, TFltPr64V& DegToCCfV, int64 SampleNodes=-1);
...
template <class PGraph> double GetClustCf(const PGraph& Graph, TFltPr64V& DegToCCfV, int64& ClosedTriadsX, int64& OpenTriadsX, int64 SampleNodes=-1);
...
template <class PGraph> int64 GetTriads(const PGraph& Graph, int64& ClosedTriadsX, int64& OpenTriadsX, int64 SampleNodes=-1);
/// Computes the number of open and close triads for every node of the network. ##TSnap::GetTriads2
template <class PGraph> void GetTriads(const PGraph& Graph, TIntTr64V& NIdCOTriadV, int64 SampleNodes=-1);
/// Counts the number of edges that participate in at least one triad. ##TSnap::GetTriadEdges
template <class PGraph> int64 GetTriadEdges(const PGraph& Graph, int64 SampleEdges=-1);

/// Returns the number of undirected triads a node \c NId participates in. ##TSnap::GetNodeTriads
template <class PGraph> int64 GetNodeTriads(const PGraph& Graph, const int64& NId);
/// Returns number of Open and Closed triads a node \c NId participates in. ##TSnap::GetNodeTriads1
template <class PGraph> int64 GetNodeTriads(const PGraph& Graph, const int64& NId, int64& ClosedNTriadsX, int64& OpenNTriadsX);
/// Returns the number of triads between a node \c NId and a subset of its neighbors \c GroupSet. ##TSnap::GetNodeTriads3
template <class PGraph>
int64 GetNodeTriads(const PGraph& Graph, const int64& NId, const TInt64Set& GroupSet, int64& InGroupEdgesX, int64& InOutGroupEdgesX, int64& OutGroupEdgesX);
...
template <class PGraph> void GetTriadParticip(const PGraph& Graph, TIntPr64V& TriadCntV);

/// Returns a number of shared neighbors between a pair of nodes NId1 and NId2.
template<class PGraph> int64 GetCmnNbrs(const PGraph& Graph, const int64& NId1, const int64& NId2);
/// Returns the shared neighbors between a pair of nodes NId1 and NId2.
template<class PGraph> int64 GetCmnNbrs(const PGraph& Graph, const int64& NId1, const int64& NId2, TInt64V& NbrV);
/// Returns the number of length 2 directed paths between a pair of nodes NId1, NId2 (NId1 --> U --> NId2).
template<class PGraph> int64 GetLen2Paths(const PGraph& Graph, const int64& NId1, const int64& NId2);
/// Returns the 2 directed paths between a pair of nodes NId1, NId2 (NId1 --> U --> NId2). ##TSnap::GetLen2Paths
template<class PGraph> int64 GetLen2Paths(const PGraph& Graph, const int64& NId1, const int64& NId2, TInt64V& NbrV);
...
template<class PGraph> void MergeNbrs(TInt64V& NeighbourV, const typename PGraph::TObj::TNodeI& NI);

/// Returns sorted vector \c NbrV containing unique in or out neighbors of node \c NId in graph \c Graph
template <class PGraph> void GetUniqueNbrV(const PGraph& Graph, const int64& NId, TInt64V& NbrV);
...
template <class PGraph> double GetClustCf(const PGraph& Graph, int64 SampleNodes) {
...
template <class PGraph> double GetClustCf(const PGraph& Graph, TFltPr64V& DegToCCfV, int64 SampleNodes) {
  TIntTr64V NIdCOTriadV;
  GetTriads(Graph, NIdCOTriadV, SampleNodes);
...
template <class PGraph>
double GetClustCf(const PGraph& Graph, TFltPr64V& DegToCCfV, int64& ClosedTriads, int64& OpenTriads, int64 SampleNodes) {
  TIntTr64V NIdCOTriadV;
  GetTriads(Graph, NIdCOTriadV, SampleNodes);
  THash<TInt64, TFltPr, int64> DegSumCnt;
  double SumCcf = 0.0;
...
template <class PGraph>
int64 GetNodeTriads(const PGraph& Graph, const int64& NId, const TInt64Set& GroupSet, int64& InGroupEdges, int64& InOutGroupEdges, int64& OutGroupEdges) {
  const typename PGraph::TObj::TNodeI NI = Graph->GetNI(NId);
  const bool IsDir = Graph->HasFlag(gfDirected);
